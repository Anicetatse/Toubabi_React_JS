...
map.on("load", function () {
  ...
  marqueurs.forEach(function (marqueur) {
    ...
    pmyl = marqueur.prix_moy_location !=null ? formatNumberWithSpaces(marqueur.prix_moy_location) : ' - ';
    ...
    pmyv = marqueur.prix_moy_vente !=null ? formatNumberWithSpaces(marqueur.prix_moy_vente) : ' - ' ;
    var popup = new mapboxgl.Popup({
      offset: 25,
    }).setHTML(
      '<div class="popup-content"><h4>' +
        marqueur.titre +
        "</h4>" +
        "<p style='font-weight:bold'>"+marqueur.nbre_biens+" bien(s)</p>" +
    ...
    marker.getElement().addEventListener("click", function () {
      ...
      tab_location = "";
      tab_vente = "";
      var biens = Object.values(marqueur.prix)
      entete =
      '<table class="table table-responsive table-striped"><tr><th>Types de biens</th><th>Prix minimum</th><th>Prix moyen</th><th>Prix maximum</th><th>Nombre de bien</th></tr>';
      biens.forEach(prix => {
     
        console.log(prix.prix_min_location)
      tab_location +=
      "<tr><td>" +
        prix.type +
        "</td><td>" +
        formatNumberWithSpaces(prix.prix_min_location??0) +
        "</td><td>" +
        formatNumberWithSpaces(prix.prix_moy_location??0) +
        "</td><td>" +
        formatNumberWithSpaces(prix.prix_max_location??0) +
        "</td><td>" +
        prix.nbre_biens_location??0 +
        "</td></tr>";
      ...
      tab_vente +=
        ...
        prix.type +
        "</td><td>" +
        formatNumberWithSpaces(prix.prix_min_vente??0) +
        "</td><td>" +
        formatNumberWithSpaces(prix.prix_moy_vente??0) +
        "</td><td>" +
        formatNumberWithSpaces(prix.prix_max_vente??0) +
        "</td><td>" +
        prix.nbre_biens_vente??0 +
        "</td></tr>";
    })
