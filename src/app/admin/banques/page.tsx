...
export default function AdminBanquesPage() {
  ...
  const [banquess, setBanques] = useState<Banque[]>([]);
  const [communes, setCommunes] = useState<Commune[]>([]);
  
  // Hooks React Query
  const { data: banquessData, isLoading } = useAdminBanques(1, 1000, '');
  ...
  const [editDialog, setEditDialog] = useState({ open: false, banques: null as Banque | null });
  const [viewDialog, setViewDialog] = useState({ open: false, banques: null as Banque | null });
  const [deleteDialog, setDeleteDialog] = useState({ open: false, banques: null as Banque | null });
  ...
  // Filtrer les banquess
  const filteredBanques = banquess.filter((p: Banque) => 
  ...
  // Trier les banquess
  const sortedBanques = [...filteredBanques].sort((a, b) => {
  ...
  const totalPages = Math.ceil(sortedBanques.length / itemsPerPage);
  const startIndex = (currentPage - 1) * itemsPerPage;
  const endIndex = startIndex + itemsPerPage;
  const paginatedBanques = sortedBanques.slice(startIndex, endIndex);

  // Statistiques
  const stats = {
    total_banquess: filteredBanques.length,
    banquess_actives: filteredBanques.filter((p: Banque) => p.active === 1).length,
    communes_couvertes: new Set(filteredBanques.filter((p: Banque) => p.commune).map((p: Banque) => p.commune?.nom)).size
  };

  // Charger les données depuis les hooks
  useEffect(() => {
    if (banquessData?.data) {
      setBanques(banquessData.data);
    }
  }, [banquessData]);
  ...
  const handleUpdate = async () => {
    if (!editDialog.banques) return;
    
    try {
      await updateMutation.mutateAsync({ 
        id: editDialog.banques.id, 
        data: formData 
      });
      setEditDialog({ open: false, banques: null });
      resetForm();
    } catch (error) {
      // L'erreur est déjà gérée par le hook
    }
  ...
  const handleDelete = async () => {
    if (!deleteDialog.banques) return;
    
    try {
      await deleteMutation.mutateAsync(deleteDialog.banques.id);
      setDeleteDialog({ open: false, banques: null });
    } catch (error) {
      // L'erreur est déjà gérée par le hook
    }
  };

  const openEditDialog = (banques: Banque) => {
    setFormData({
      id_commune: banques.id_commune?.toString() || '',
      nom: banques.nom || '',
      description: banques.description || '',
      contact: banques.contact || '',
      active: banques.active.toString()
    });
    setEditDialog({ open: true, banques });
  };
  ...
  const downloadTemplate = () => {
    ...
    const data = [
      ...
      ['Banque Exemple', '77 123 45 67', 'Description de la banques', '1'],
      ['Banque Express', '77 987 65 43', 'Autre description', '1']
    ];
    ...
    XLSX.writeFile(wb, 'template_banquess.xlsx');
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Importer les banquess une par une (sans toast individuel)
      let imported = 0;
      let errors = 0;
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        console.log(`➡️ Import ${i + 1}/${data.length}:`, banquesData);
        
        try {
          // Utiliser l'API directement sans passer par le hook pour éviter les toasts
          const token = localStorage.getItem('admin_token');
          await fetch('/api/admin/banquess', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
            },
            body: JSON.stringify(banquesData)
          });
      ...
      if (errors > 0) {
        toast.success(`${imported} banques(s) importée(s), ${errors} erreur(s)`, { duration: 5000 });
      } else {
        toast.success(`${imported} banques(s) importée(s) avec succès`, { duration: 3000 });
      }
    ...
    return (
      <AdminLayout>
        <div className="flex items-center justify-center h-64">
          ...
          <p className="ml-4 text-gray-600">Chargement des banquess...</p>
        </div>
      </AdminLayout>
    );
  }
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Gérez les banquess bancaire par commune
            </p>
          </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="relative">
              ...
              <Input
                placeholder="Rechercher une banques (nom, commune, contact)..."
                value={searchQuery}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">Total Banques</p>
                  <p className="text-2xl font-bold text-gray-900">{stats.total_banquess}</p>
                </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">Banques Actives</p>
                  <p className="text-2xl font-bold text-gray-900">{stats.banquess_actives}</p>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Liste des Banques ({filteredBanques.length})</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedBanques.length > 0 ? (
                    paginatedBanques.map((banques: Banque) => (
                      <tr key={banques.id} className="border-b hover:bg-gray-50">
                        <td className="py-4 px-4">
                          <p className="font-medium text-gray-900">{banques.nom || 'N/A'}</p>
                        </td>
                        <td className="py-4 px-4">
                          <p className="text-sm text-gray-900">{banques.commune?.nom || 'N/A'}</p>
                        </td>
                        <td className="py-4 px-4">
                          <span className="text-sm text-gray-700">{banques.contact || '-'}</span>
                        </td>
                        <td className="py-4 px-4 max-w-xs">
                          {banques.description ? (
                            <div 
                              className="text-sm text-gray-700 line-clamp-2 prose prose-sm max-w-none"
                              dangerouslySetInnerHTML={{ __html: banques.description }}
                            />
                          ) : (
                        ...
                        <td className="py-4 px-4 text-center">
                          <Badge 
                            variant="outline" 
                            className={banques.active === 1 
                              ? 'bg-green-50 text-green-700 border-green-200' 
                              : 'bg-gray-50 text-gray-700 border-gray-200'
                            }
                          >
                            {banques.active === 1 ? 'Oui' : 'Non'}
                          </Badge>
                        </td>
                        <td className="py-4 px-4">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => setViewDialog({ open: true, banques })}>
                                <Eye className="mr-2 h-4 w-4" />
                                Voir les détails
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => openEditDialog(banques)}>
                                <Edit className="mr-2 h-4 w-4" />
                                Modifier
                              </DropdownMenuItem>
                              <DropdownMenuItem 
                                onClick={() => setDeleteDialog({ open: true, banques })}
                    ...
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-gray-400">
                        ...
                        <p className="text-lg font-medium">Aucune banques trouvée</p>
                        <p className="text-sm">Commencez par ajouter une banques bancaire</p>
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            {totalPages > 1 && (
              <div className="mt-6 flex justify-center">
                <Pagination
                  ...
                  totalItems={sortedBanques.length}
        ...
        <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-base text-gray-600">
              Ajoutez une nouvelle banques bancaire
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-6 py-4">
            ...
            <div className="space-y-2">
              ...
              <RichTextEditor
                ...
                placeholder="Entrez la description de la banques..."
              />
            </div>
      ...
      <Dialog open={editDialog.open} onOpenChange={(open: boolean) => !open && setEditDialog({ open: false, banques: null })}>
        <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            <DialogTitle className="text-2xl font-bold flex items-center gap-2">
            ...
            <DialogDescription className="text-base text-gray-600">
              Modifiez les informations de la banques
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open: boolean) => setDeleteDialog({ open, banques: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmer la suppression</AlertDialogTitle>
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir supprimer la banques <strong>{deleteDialog.banques?.nom}</strong> ?
              Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeleteDialog({ open: false, banques: null })}>
              Annuler
            </AlertDialogCancel>
            <AlertDialogAction 
      ...
      <Dialog open={viewDialog.open} onOpenChange={(open: boolean) => setViewDialog({ open, banques: null })}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Informations complètes de la banques
            </DialogDescription>
          </DialogHeader>
          ...
          {viewDialog.banques && (
            <div className="space-y-4 py-4">
              ...
              <div className="bg-gradient-to-r from-red-50 to-orange-50 p-4 rounded-lg border border-red-100">
                ...
                <p className="font-semibold text-gray-900">{viewDialog.banques.nom || 'N/A'}</p>
              </div>

              {/* Commune */}
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-100">
                <p className="text-sm text-gray-600 mb-1">Commune</p>
                <p className="font-semibold text-gray-900">{viewDialog.banques.commune?.nom || 'N/A'}</p>
              </div>

              {/* Contact */}
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-100">
                <p className="text-sm text-gray-600 mb-1">Contact</p>
                <p className="font-semibold text-gray-900">{viewDialog.banques.contact || '-'}</p>
              </div>

              {/* Description */}
              {viewDialog.banques.description && (
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg border border-purple-100">
                  ...
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: viewDialog.banques.description }}
                  />
                </div>
              )}
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                ...
                <Badge 
                  variant="outline" 
                  className={viewDialog.banques.active === 1 
                    ? 'bg-green-50 text-green-700 border-green-200' 
                    : 'bg-gray-50 text-gray-700 border-gray-200'
                  }
                >
                  {viewDialog.banques.active === 1 ? 'Oui' : 'Non'}
                </Badge>
              </div>
            </div>
          )}
          ...
          <DialogFooter>
            <Button 
              variant="outline" 
              onClick={() => setViewDialog({ open: false, banques: null })}
              className="px-6"
            >
              Fermer
            </Button>
      ...
      <Dialog open={importDialog.open} onOpenChange={(open: boolean) => setImportDialog({ open })}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-base text-gray-600">
              Importez plusieurs banquess depuis un fichier Excel
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-red-700 mb-3">
                    Toutes les banquess importées seront assignées à cette commune
                  </p>
                  <SearchableSelect
                    options={communes.map((commune: Commune) => ({ 
                      value: commune.id.toString(), 
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <div className="bg-white rounded border border-gray-200 p-3 font-mono text-xs text-gray-700 overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    <tr className="border-b">
                      <td className="px-2 py-1">Banque Centrale</td>
                      <td className="px-2 py-1">77 123 45 67</td>
                      <td className="px-2 py-1">Description...</td>
                      <td className="px-2 py-1">1</td>
                    </tr>
                    <tr>
                      <td className="px-2 py-1">Banque Express</td>
