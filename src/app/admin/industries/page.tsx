...
export default function AdminIndustriesPage() {
  ...
  const [industriess, setIndustries] = useState<Industrie[]>([]);
  const [communes, setCommunes] = useState<Commune[]>([]);
  
  // Hooks React Query
  const { data: industriessData, isLoading } = useAdminIndustries(1, 1000, '');
  const { data: referencesData } = usePrixReferences(); // Pour charger les communes
  const createMutation = useCreateIndustrie();
  const updateMutation = useUpdateIndustrie();
  const deleteMutation = useDeleteIndustrie();
  
  // États pour les dialogues
  const [createDialog, setCreateDialog] = useState({ open: false });
  const [editDialog, setEditDialog] = useState({ open: false, industries: null as Industrie | null });
  const [viewDialog, setViewDialog] = useState({ open: false, industries: null as Industrie | null });
  const [deleteDialog, setDeleteDialog] = useState({ open: false, industries: null as Industrie | null });
  ...
  // Filtrer les industriess
  const filteredIndustries = industriess.filter((p: Industrie) => 
  ...
  // Statistiques
  const stats = {
    total_industriess: filteredIndustries.length,
    industriess_actives: filteredIndustries.filter((p: Industrie) => p.active === 1).length,
    communes_couvertes: new Set(filteredIndustries.filter((p: Industrie) => p.commune).map((p: Industrie) => p.commune?.nom)).size
  };

  // Charger les données depuis les hooks
  useEffect(() => {
    if (industriessData?.data) {
      setIndustries(industriessData.data);
    }
  }, [industriessData]);
  ...
  const handleUpdate = async () => {
    if (!editDialog.industries) return;
    
    try {
      await updateMutation.mutateAsync({ 
        id: editDialog.industries.id, 
        data: formData 
      });
      setEditDialog({ open: false, industries: null });
      resetForm();
    } catch (error) {
      // L'erreur est déjà gérée par le hook
    }
  ...
  const handleDelete = async () => {
    if (!deleteDialog.industries) return;
    
    try {
      await deleteMutation.mutateAsync(deleteDialog.industries.id);
      setDeleteDialog({ open: false, industries: null });
    } catch (error) {
      // L'erreur est déjà gérée par le hook
    }
  };

  const openEditDialog = (industries: Industrie) => {
    setFormData({
      id_commune: industries.id_commune?.toString() || '',
      nom: industries.nom || '',
      description: industries.description || '',
      contact: industries.contact || '',
      active: industries.active.toString()
    });
    setEditDialog({ open: true, industries });
  };
  ...
  const downloadTemplate = () => {
    ...
    const data = [
      ...
      ['Industrie Exemple', '77 123 45 67', 'Description de la industries', '1'],
      ['Industrie Express', '77 987 65 43', 'Autre description', '1']
    ];
    ...
    XLSX.writeFile(wb, 'template_industriess.xlsx');
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Importer les industriess une par une (sans toast individuel)
      let imported = 0;
      let errors = 0;
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        console.log(`➡️ Import ${i + 1}/${data.length}:`, industriesData);
        
        try {
          // Utiliser l'API directement sans passer par le hook pour éviter les toasts
          const token = localStorage.getItem('admin_token');
          await fetch('/api/admin/industriess', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${token}`
            },
            body: JSON.stringify(industriesData)
          });
      ...
      if (errors > 0) {
        toast.success(`${imported} industries(s) importée(s), ${errors} erreur(s)`, { duration: 5000 });
      } else {
        toast.success(`${imported} industries(s) importée(s) avec succès`, { duration: 3000 });
      }
    ...
    return (
      <AdminLayout>
        <div className="flex items-center justify-center h-64">
          ...
          <p className="ml-4 text-gray-600">Chargement des industriess...</p>
        </div>
      </AdminLayout>
    );
  }
    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Gestion des Industries</h1>
            <p className="text-gray-600">
              Gérez les industriess active par commune
            </p>
          </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="relative">
              ...
              <Input
                placeholder="Rechercher une industries (nom, commune, contact)..."
                value={searchQuery}
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{stats.total_industriess}</p>
                </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-gray-600">Industries Actives</p>
                  <p className="text-2xl font-bold text-gray-900">{stats.industriess_actives}</p>
                </div>
        ...
        {/* Liste des industriess */}
        <Card>
          <CardHeader>
            <CardTitle>Liste des Industries ({filteredIndustries.length})</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedIndustries.length > 0 ? (
                    paginatedIndustries.map((industries: Industrie) => (
                      <tr key={industries.id} className="border-b hover:bg-gray-50">
                        <td className="py-4 px-4">
                          <p className="font-medium text-gray-900">{industries.nom || 'N/A'}</p>
                        </td>
                        <td className="py-4 px-4">
                          <p className="text-sm text-gray-900">{industries.commune?.nom || 'N/A'}</p>
                        </td>
                        <td className="py-4 px-4">
                          <span className="text-sm text-gray-700">{industries.contact || '-'}</span>
                        </td>
                        <td className="py-4 px-4 max-w-xs">
                          {industries.description ? (
                            <div 
                              className="text-sm text-gray-700 line-clamp-2 prose prose-sm max-w-none"
                              dangerouslySetInnerHTML={{ __html: industries.description }}
                            />
                          ) : (
                        ...
                        <td className="py-4 px-4 text-center">
                          <Badge 
                            variant="outline" 
                            className={industries.active === 1 
                              ? 'bg-green-50 text-green-700 border-green-200' 
                              : 'bg-gray-50 text-gray-700 border-gray-200'
                            }
                          >
                            {industries.active === 1 ? 'Oui' : 'Non'}
                          </Badge>
                        </td>
                        <td className="py-4 px-4">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => setViewDialog({ open: true, industries })}>
                                <Eye className="mr-2 h-4 w-4" />
                                Voir les détails
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => openEditDialog(industries)}>
                                <Edit className="mr-2 h-4 w-4" />
                                Modifier
                              </DropdownMenuItem>
                              <DropdownMenuItem 
                                onClick={() => setDeleteDialog({ open: true, industries })}
                    ...
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-gray-400">
                        ...
                        <p className="text-lg font-medium">Aucune industries trouvée</p>
                        <p className="text-sm">Commencez par ajouter une industries active</p>
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            {totalPages > 1 && (
              <div className="mt-6 flex justify-center">
                <Pagination
                  ...
                  totalItems={sortedIndustries.length}
        ...
        <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-base text-gray-600">
              Ajoutez une nouvelle industries active
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-6 py-4">
            ...
            <div className="space-y-2">
              <Label htmlFor="nom" className="text-sm font-medium flex items-center gap-2">
                ...
                Nom de la industries *
              </Label>
              <Input
                id="nom"
                value={formData.nom}
                onChange={(e) => setFormData({ ...formData, nom: e.target.value })}
                placeholder="Industrie Centrale"
                className="h-10"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <RichTextEditor
                ...
                placeholder="Entrez la description de la industries..."
              />
            </div>
      ...
      <Dialog open={editDialog.open} onOpenChange={(open: boolean) => !open && setEditDialog({ open: false, industries: null })}>
        <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            <DialogTitle className="text-2xl font-bold flex items-center gap-2">
              ...
              Modifier la Industrie
            </DialogTitle>
            <DialogDescription className="text-base text-gray-600">
              Modifiez les informations de la industries
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open: boolean) => setDeleteDialog({ open, industries: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmer la suppression</AlertDialogTitle>
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir supprimer la industries <strong>{deleteDialog.industries?.nom}</strong> ?
              Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeleteDialog({ open: false, industries: null })}>
              Annuler
            </AlertDialogCancel>
            <AlertDialogAction 
      ...
      <Dialog open={viewDialog.open} onOpenChange={(open: boolean) => setViewDialog({ open, industries: null })}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="text-2xl font-bold flex items-center gap-2">
              ...
              Détails de la Industrie
            </DialogTitle>
            <DialogDescription>
              Informations complètes de la industries
            </DialogDescription>
          </DialogHeader>
          
          {viewDialog.industries && (
            <div className="space-y-4 py-4">
              {/* Nom */}
              <div className="bg-gradient-to-r from-red-50 to-orange-50 p-4 rounded-lg border border-red-100">
                <p className="text-sm text-gray-600 mb-1">Nom</p>
                <p className="font-semibold text-gray-900">{viewDialog.industries.nom || 'N/A'}</p>
              </div>

              {/* Commune */}
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-100">
                <p className="text-sm text-gray-600 mb-1">Commune</p>
                <p className="font-semibold text-gray-900">{viewDialog.industries.commune?.nom || 'N/A'}</p>
              </div>

              {/* Contact */}
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-100">
                <p className="text-sm text-gray-600 mb-1">Contact</p>
                <p className="font-semibold text-gray-900">{viewDialog.industries.contact || '-'}</p>
              </div>

              {/* Description */}
              {viewDialog.industries.description && (
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg border border-purple-100">
                  ...
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: viewDialog.industries.description }}
                  />
                </div>
              )}
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                ...
                <Badge 
                  variant="outline" 
                  className={viewDialog.industries.active === 1 
                    ? 'bg-green-50 text-green-700 border-green-200' 
                    : 'bg-gray-50 text-gray-700 border-gray-200'
                  }
                >
                  {viewDialog.industries.active === 1 ? 'Oui' : 'Non'}
                </Badge>
              </div>
            </div>
          )}
          ...
          <DialogFooter>
            <Button 
              variant="outline" 
              onClick={() => setViewDialog({ open: false, industries: null })}
              className="px-6"
            >
              Fermer
            </Button>
      ...
      <Dialog open={importDialog.open} onOpenChange={(open: boolean) => setImportDialog({ open })}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-base text-gray-600">
              Importez plusieurs industriess depuis un fichier Excel
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-red-700 mb-3">
                    Toutes les industriess importées seront assignées à cette commune
                  </p>
                  <SearchableSelect
                    options={communes.map((commune: Commune) => ({ 
                      value: commune.id.toString(), 
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <div className="bg-white rounded border border-gray-200 p-3 font-mono text-xs text-gray-700 overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    <tr className="border-b">
                      <td className="px-2 py-1">Industrie Centrale</td>
                      <td className="px-2 py-1">77 123 45 67</td>
                      <td className="px-2 py-1">Description...</td>
                      <td className="px-2 py-1">1</td>
                    </tr>
                    <tr>
                      <td className="px-2 py-1">Industrie Express</td>
