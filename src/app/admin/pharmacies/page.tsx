...
export default function AdminPharmaciesPage() {
  ...
  // Hooks React Query
  const { data: pharmaciesData, isLoading } = useAdminPharmacies(1, 1000, '');
  const { data: referencesData } = usePrixReferences(); // Pour charger les communes
  const createMutation = useCreatePharmacie();
  const updateMutation = useUpdatePharmacie();
  const deleteMutation = useDeletePharmacie();
  
  // États pour les dialogues
  const [createDialog, setCreateDialog] = useState({ open: false });
  const [editDialog, setEditDialog] = useState({ open: false, pharmacie: null as Pharmacie | null });
  const [viewDialog, setViewDialog] = useState({ open: false, pharmacie: null as Pharmacie | null });
  const [deleteDialog, setDeleteDialog] = useState({ open: false, pharmacie: null as Pharmacie | null });
  ...
  const filteredPharmacies = pharmacies.filter((p: Pharmacie) => 
  ...
  const paginatedPharmacies = sortedPharmacies.slice(startIndex, endIndex);

  // Statistiques
  const stats = {
    total_pharmacies: filteredPharmacies.length,
    pharmacies_actives: filteredPharmacies.filter((p: Pharmacie) => p.active === 1).length,
    communes_couvertes: new Set(filteredPharmacies.filter((p: Pharmacie) => p.commune).map((p: Pharmacie) => p.commune?.nom)).size
  };
  ...
  const handleUpdate = async () => {
    if (!editDialog.pharmacie) return;
    
    try {
      await updateMutation.mutateAsync({ 
        id: editDialog.pharmacie.id, 
        data: formData 
      });
      setEditDialog({ open: false, pharmacie: null });
      resetForm();
    } catch (error) {
      // L'erreur est déjà gérée par le hook
    }
  ...
  const handleDelete = async () => {
    if (!deleteDialog.pharmacie) return;
    
    try {
      await deleteMutation.mutateAsync(deleteDialog.pharmacie.id);
      setDeleteDialog({ open: false, pharmacie: null });
    } catch (error) {
      // L'erreur est déjà gérée par le hook
    }
  };

  const openEditDialog = (pharmacie: Pharmacie) => {
    setFormData({
      id_commune: pharmacie.id_commune?.toString() || '',
      nom: pharmacie.nom || '',
      description: pharmacie.description || '',
      contact: pharmacie.contact || '',
      active: pharmacie.active.toString()
    });
    setEditDialog({ open: true, pharmacie });
  };
  ...
  const downloadTemplate = () => {
    ...
    const data = [
      ...
      ['Pharmacie Exemple', '77 123 45 67', 'Description de la pharmacie', '1'],
      ['Pharmacie Express', '77 987 65 43', 'Autre description', '1']
    ];
    ...
    XLSX.writeFile(wb, 'template_pharmacies.xlsx');
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Importer les pharmacies une par une (sans toast individuel)
      let imported = 0;
      let errors = 0;
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        console.log(`➡️ Import ${i + 1}/${data.length}:`, pharmacieData);
      ...
      if (errors > 0) {
        toast.success(`${imported} pharmacie(s) importée(s), ${errors} erreur(s)`, { duration: 5000 });
      } else {
        toast.success(`${imported} pharmacie(s) importée(s) avec succès`, { duration: 3000 });
      }
    ...
    return (
      <AdminLayout>
        <div className="flex items-center justify-center h-64">
          ...
          <p className="ml-4 text-gray-600">Chargement des pharmacies...</p>
        </div>
      </AdminLayout>
    );
  }
  ...
  return (
    <AdminLayout>
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Gérez les pharmacies de garde par commune
            </p>
          </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="relative">
              ...
              <Input
                placeholder="Rechercher une pharmacie (nom, commune, contact)..."
                value={searchQuery}
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{stats.total_pharmacies}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{stats.pharmacies_actives}</p>
                </div>
              ...
              <table className="w-full">
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-center py-3 px-4 font-medium text-gray-600">De garde</th>
                    <th className="text-left py-3 px-4 font-medium text-gray-600">Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {paginatedPharmacies.length > 0 ? (
                    paginatedPharmacies.map((pharmacie: Pharmacie) => (
                      <tr key={pharmacie.id} className="border-b hover:bg-gray-50">
                        <td className="py-4 px-4">
                          <p className="font-medium text-gray-900">{pharmacie.nom || 'N/A'}</p>
                        </td>
                        <td className="py-4 px-4">
                          <p className="text-sm text-gray-900">{pharmacie.commune?.nom || 'N/A'}</p>
                        </td>
                        <td className="py-4 px-4">
                          <span className="text-sm text-gray-700">{pharmacie.contact || '-'}</span>
                        </td>
                        <td className="py-4 px-4 max-w-xs">
                          {pharmacie.description ? (
                            <div 
                              className="text-sm text-gray-700 line-clamp-2 prose prose-sm max-w-none"
                              dangerouslySetInnerHTML={{ __html: pharmacie.description }}
                            />
                          ) : (
                        ...
                        <td className="py-4 px-4 text-center">
                          <Badge 
                            variant="outline" 
                            className={pharmacie.active === 1 
                              ? 'bg-green-50 text-green-700 border-green-200' 
                              : 'bg-gray-50 text-gray-700 border-gray-200'
                            }
                          >
                            {pharmacie.active === 1 ? 'Oui' : 'Non'}
                          </Badge>
                        </td>
                        <td className="py-4 px-4">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => setViewDialog({ open: true, pharmacie })}>
                                <Eye className="mr-2 h-4 w-4" />
                                Voir les détails
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => openEditDialog(pharmacie)}>
                                <Edit className="mr-2 h-4 w-4" />
                                Modifier
                              </DropdownMenuItem>
                              <DropdownMenuItem 
                                onClick={() => setDeleteDialog({ open: true, pharmacie })}
                    ...
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-gray-400">
                        ...
                        <p className="text-sm">Commencez par ajouter une pharmacie de garde</p>
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            {totalPages > 1 && (
              <div className="mt-6 flex justify-center">
                <Pagination
                  ...
                  totalItems={sortedPharmacies.length}
        ...
        <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-base text-gray-600">
              Ajoutez une nouvelle pharmacie de garde
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-6 py-4">
            ...
            <div className="space-y-2">
              ...
              <RichTextEditor
                ...
                placeholder="Entrez la description de la pharmacie..."
              />
            </div>
      ...
      <Dialog open={editDialog.open} onOpenChange={(open: boolean) => !open && setEditDialog({ open: false, pharmacie: null })}>
        <DialogContent className="sm:max-w-[550px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            <DialogTitle className="text-2xl font-bold flex items-center gap-2">
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open: boolean) => setDeleteDialog({ open, pharmacie: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmer la suppression</AlertDialogTitle>
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir supprimer la pharmacie <strong>{deleteDialog.pharmacie?.nom}</strong> ?
              Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeleteDialog({ open: false, pharmacie: null })}>
              Annuler
            </AlertDialogCancel>
            <AlertDialogAction 
      ...
      <Dialog open={viewDialog.open} onOpenChange={(open: boolean) => setViewDialog({ open, pharmacie: null })}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Informations complètes de la pharmacie
            </DialogDescription>
          </DialogHeader>
          
          {viewDialog.pharmacie && (
            <div className="space-y-4 py-4">
              {/* Nom */}
              <div className="bg-gradient-to-r from-red-50 to-orange-50 p-4 rounded-lg border border-red-100">
                <p className="text-sm text-gray-600 mb-1">Nom</p>
                <p className="font-semibold text-gray-900">{viewDialog.pharmacie.nom || 'N/A'}</p>
              </div>

              {/* Commune */}
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-100">
                <p className="text-sm text-gray-600 mb-1">Commune</p>
                <p className="font-semibold text-gray-900">{viewDialog.pharmacie.commune?.nom || 'N/A'}</p>
              </div>

              {/* Contact */}
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-100">
                <p className="text-sm text-gray-600 mb-1">Contact</p>
                <p className="font-semibold text-gray-900">{viewDialog.pharmacie.contact || '-'}</p>
              </div>

              {/* Description */}
              {viewDialog.pharmacie.description && (
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg border border-purple-100">
                  ...
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: viewDialog.pharmacie.description }}
                  />
                </div>
              )}
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                <p className="text-sm text-gray-600 mb-2">De garde</p>
                <Badge 
                  variant="outline" 
                  className={viewDialog.pharmacie.active === 1 
                    ? 'bg-green-50 text-green-700 border-green-200' 
                    : 'bg-gray-50 text-gray-700 border-gray-200'
                  }
                >
                  {viewDialog.pharmacie.active === 1 ? 'Oui' : 'Non'}
                </Badge>
              </div>
            </div>
          )}
          ...
          <DialogFooter>
            <Button 
              variant="outline" 
              onClick={() => setViewDialog({ open: false, pharmacie: null })}
              className="px-6"
            >
              Fermer
            </Button>
      ...
      <Dialog open={importDialog.open} onOpenChange={(open: boolean) => setImportDialog({ open })}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-base text-gray-600">
              Importez plusieurs pharmacies depuis un fichier Excel
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-red-700 mb-3">
                    Toutes les pharmacies importées seront assignées à cette commune
                  </p>
                  <SearchableSelect
                    options={communes.map((commune: Commune) => ({ 
                      value: commune.id.toString(), 
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <div className="bg-white rounded border border-gray-200 p-3 font-mono text-xs text-gray-700 overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    <tr className="border-b">
                      <td className="px-2 py-1">Pharmacie Centrale</td>
                      <td className="px-2 py-1">77 123 45 67</td>
                      <td className="px-2 py-1">Description...</td>
                      <td className="px-2 py-1">1</td>
                    </tr>
                    <tr>
                      <td className="px-2 py-1">Pharmacie Express</td>
