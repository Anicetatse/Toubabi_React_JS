...
export default function AdminStationsPage() {
  ...
  const [stationss, setStations] = useState<Station[]>([]);
  const [communes, setCommunes] = useState<Commune[]>([]);
  
  // Hooks React Query
  const { data: stationssData, isLoading } = useAdminStations(1, 1000, '');
  ...
  const [editDialog, setEditDialog] = useState({ open: false, stations: null as Station | null });
  const [viewDialog, setViewDialog] = useState({ open: false, stations: null as Station | null });
  const [deleteDialog, setDeleteDialog] = useState({ open: false, stations: null as Station | null });
  ...
  // Filtrer les stationss
  const filteredStations = stationss.filter((p: Station) => 
  ...
  // Statistiques
  const stats = {
    total_stationss: filteredStations.length,
    stationss_actives: filteredStations.filter((p: Station) => p.active === 1).length,
    communes_couvertes: new Set(filteredStations.filter((p: Station) => p.commune).map((p: Station) => p.commune?.nom)).size
  };

  // Charger les données depuis les hooks
  useEffect(() => {
    if (stationssData?.data) {
      setStations(stationssData.data);
    }
  }, [stationssData]);
  ...
  const handleUpdate = async () => {
    if (!editDialog.stations) return;
    
    try {
      await updateMutation.mutateAsync({ 
        id: editDialog.stations.id, 
        data: formData 
      });
      setEditDialog({ open: false, stations: null });
      resetForm();
    } catch (error) {
      // L'erreur est déjà gérée par le hook
    }
  ...
  const handleDelete = async () => {
    if (!deleteDialog.stations) return;
    
    try {
      await deleteMutation.mutateAsync(deleteDialog.stations.id);
      setDeleteDialog({ open: false, stations: null });
    } catch (error) {
      // L'erreur est déjà gérée par le hook
    }
  };

  const openEditDialog = (stations: Station) => {
    setFormData({
      id_commune: stations.id_commune?.toString() || '',
      nom: stations.nom || '',
      description: stations.description || '',
      contact: stations.contact || '',
      active: stations.active.toString()
    });
    setEditDialog({ open: true, stations });
  };
  ...
  const downloadTemplate = () => {
    ...
    const data = [
      ...
      ['Station Exemple', '77 123 45 67', 'Description de la stations', '1'],
      ['Station Express', '77 987 65 43', 'Autre description', '1']
    ];
    ...
    XLSX.writeFile(wb, 'template_stationss.xlsx');
  };
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      // Importer les stationss une par une (sans toast individuel)
      let imported = 0;
      let errors = 0;
      ...
      if (errors > 0) {
        toast.success(`${imported} stations(s) importée(s), ${errors} erreur(s)`, { duration: 5000 });
      } else {
        toast.success(`${imported} stations(s) importée(s) avec succès`, { duration: 3000 });
      }
    ...
    return (
      <AdminLayout>
        <div className="flex items-center justify-center h-64">
          ...
          <p className="ml-4 text-gray-600">Chargement des stationss...</p>
        </div>
      </AdminLayout>
    );
  }
  ...
  return (
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-gray-600">
              Gérez les stationss service par commune
            </p>
          </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="relative">
              ...
              <Input
                placeholder="Rechercher une stations (nom, commune, contact)..."
                value={searchQuery}
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{stats.total_stationss}</p>
                </div>
            ...
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">{stats.stationss_actives}</p>
                </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Liste des Stations ({filteredStations.length})</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {paginatedStations.length > 0 ? (
                    paginatedStations.map((stations: Station) => (
                      <tr key={stations.id} className="border-b hover:bg-gray-50">
                        <td className="py-4 px-4">
                          <p className="font-medium text-gray-900">{stations.nom || 'N/A'}</p>
                        </td>
                        <td className="py-4 px-4">
                          <p className="text-sm text-gray-900">{stations.commune?.nom || 'N/A'}</p>
                        </td>
                        <td className="py-4 px-4">
                          <span className="text-sm text-gray-700">{stations.contact || '-'}</span>
                        </td>
                        <td className="py-4 px-4 max-w-xs">
                          {stations.description ? (
                            <div 
                              className="text-sm text-gray-700 line-clamp-2 prose prose-sm max-w-none"
                              dangerouslySetInnerHTML={{ __html: stations.description }}
                            />
                          ) : (
                        ...
                        <td className="py-4 px-4 text-center">
                          <Badge 
                            variant="outline" 
                            className={stations.active === 1 
                              ? 'bg-green-50 text-green-700 border-green-200' 
                              : 'bg-gray-50 text-gray-700 border-gray-200'
                            }
                          >
                            {stations.active === 1 ? 'Oui' : 'Non'}
                          </Badge>
                        </td>
                        <td className="py-4 px-4">
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              <DropdownMenuItem onClick={() => setViewDialog({ open: true, stations })}>
                                <Eye className="mr-2 h-4 w-4" />
                                Voir les détails
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => openEditDialog(stations)}>
                                <Edit className="mr-2 h-4 w-4" />
                                Modifier
                              </DropdownMenuItem>
                              <DropdownMenuItem 
                                onClick={() => setDeleteDialog({ open: true, stations })}
                    ...
                    <tr>
                      <td colSpan={6} className="py-12 text-center text-gray-400">
                        ...
                        <p className="text-lg font-medium">Aucune stations trouvée</p>
                        <p className="text-sm">Commencez par ajouter une stations service</p>
                      </td>
                    </tr>
                  )}
                </tbody>
            ...
            {totalPages > 1 && (
              <div className="mt-6 flex justify-center">
                <Pagination
                  ...
                  totalItems={sortedStations.length}
        ...
        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-base text-gray-600">
              Ajoutez une nouvelle stations service
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="grid gap-6 py-4">
            ...
            <div className="space-y-2">
              ...
              <RichTextEditor
                ...
                placeholder="Entrez la description de la stations..."
              />
            </div>
      ...
      <Dialog open={editDialog.open} onOpenChange={(open: boolean) => !open && setEditDialog({ open: false, stations: null })}>
        <DialogContent className="sm:max-w-[900px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-base text-gray-600">
              Modifiez les informations de la stations
            </DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={deleteDialog.open} onOpenChange={(open: boolean) => setDeleteDialog({ open, stations: null })}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmer la suppression</AlertDialogTitle>
            <AlertDialogDescription>
              Êtes-vous sûr de vouloir supprimer la stations <strong>{deleteDialog.stations?.nom}</strong> ?
              Cette action est irréversible.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel onClick={() => setDeleteDialog({ open: false, stations: null })}>
              Annuler
            </AlertDialogCancel>
            <AlertDialogAction 
      ...
      <Dialog open={viewDialog.open} onOpenChange={(open: boolean) => setViewDialog({ open, stations: null })}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Informations complètes de la stations
            </DialogDescription>
          </DialogHeader>
          ...
          {viewDialog.stations && (
            <div className="space-y-4 py-4">
              ...
              <div className="bg-gradient-to-r from-red-50 to-orange-50 p-4 rounded-lg border border-red-100">
                ...
                <p className="font-semibold text-gray-900">{viewDialog.stations.nom || 'N/A'}</p>
              </div>

              {/* Commune */}
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 p-4 rounded-lg border border-blue-100">
                <p className="text-sm text-gray-600 mb-1">Commune</p>
                <p className="font-semibold text-gray-900">{viewDialog.stations.commune?.nom || 'N/A'}</p>
              </div>

              {/* Contact */}
              <div className="bg-gradient-to-r from-green-50 to-emerald-50 p-4 rounded-lg border border-green-100">
                <p className="text-sm text-gray-600 mb-1">Contact</p>
                <p className="font-semibold text-gray-900">{viewDialog.stations.contact || '-'}</p>
              </div>

              {/* Description */}
              {viewDialog.stations.description && (
                <div className="bg-gradient-to-r from-purple-50 to-pink-50 p-4 rounded-lg border border-purple-100">
                  ...
                  <div 
                    ...
                    dangerouslySetInnerHTML={{ __html: viewDialog.stations.description }}
                  />
                </div>
              )}
              ...
              <div className="bg-gray-50 p-4 rounded-lg border border-gray-100">
                ...
                <Badge 
                  variant="outline" 
                  className={viewDialog.stations.active === 1 
                    ? 'bg-green-50 text-green-700 border-green-200' 
                    : 'bg-gray-50 text-gray-700 border-gray-200'
                  }
                >
                  {viewDialog.stations.active === 1 ? 'Oui' : 'Non'}
                </Badge>
              </div>
            </div>
          )}
          ...
          <DialogFooter>
            <Button 
              variant="outline" 
              onClick={() => setViewDialog({ open: false, stations: null })}
              className="px-6"
            >
              Fermer
            </Button>
      ...
      <Dialog open={importDialog.open} onOpenChange={(open: boolean) => setImportDialog({ open })}>
        <DialogContent className="sm:max-w-[800px] max-h-[90vh] overflow-y-auto">
          <DialogHeader className="space-y-3">
            ...
            <DialogDescription className="text-base text-gray-600">
              Importez plusieurs stationss depuis un fichier Excel
            </DialogDescription>
          </DialogHeader>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm text-red-700 mb-3">
                    Toutes les stationss importées seront assignées à cette commune
                  </p>
                  <SearchableSelect
                    options={communes.map((commune: Commune) => ({ 
                      value: commune.id.toString(), 
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              ...
              <div className="bg-white rounded border border-gray-200 p-3 font-mono text-xs text-gray-700 overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody>
                    <tr className="border-b">
                      <td className="px-2 py-1">Station Centrale</td>
                      <td className="px-2 py-1">77 123 45 67</td>
                      <td className="px-2 py-1">Description...</td>
                      <td className="px-2 py-1">1</td>
                    </tr>
                    <tr>
                      <td className="px-2 py-1">Station Express</td>
