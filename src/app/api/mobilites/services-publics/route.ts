...
export async function GET(request: NextRequest) {
  try {
    ...
    const communeIds = services.map(s => s.id_commune).filter((id): id is number => id !== null);
    const communes = communeIds.length > 0 ? await prisma.communes.findMany({
      where: { id: { in: communeIds.map(id => BigInt(id)) } }
    }) : [];

    const servicesWithCommunes = services.map(service => ({
      ...service,
      commune: service.id_commune ? communes.find(c => c.id === BigInt(service.id_commune!)) : null
    }));

    return NextResponse.json(serializeBigInt({
      success: true,
      data: servicesWithCommunes,
      pagination: {
        current_page: page,
        per_page: limit,
        total: total,
