...
export async function GET(request: NextRequest) {
  try {
    ...
    const communeIds = commerces.map(c => c.id_commune).filter((id): id is number => id !== null);
    ...
    const commercesWithCommunes = commerces.map(commerce => ({
      ...commerce,
      commune: commerce.id_commune ? communes.find(c => c.id === BigInt(commerce.id_commune!)) : null
    }));
