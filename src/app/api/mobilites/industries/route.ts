...
export async function GET(request: NextRequest) {
  try {
    ...
    const communeIds = industries.map(i => i.id_commune).filter((id): id is number => id !== null);
    ...
    const industriesWithCommunes = industries.map(industrie => ({
      ...industrie,
      commune: industrie.id_commune ? communes.find(c => c.id === BigInt(industrie.id_commune!)) : null
    }));
